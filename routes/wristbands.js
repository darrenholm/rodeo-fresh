...
router.post('/link', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      RETURNING *`,
      [id, rfid_uid, ticket_order_id, ticket.customer_name, ticket.customer_email, ticket_type || 'adult']
    );
    
    console.log(`✓ Wristband linked: ${rfid_uid} → ${ticket_order_id}`);
...
router.post('/approve-age', authenticateToken, async (req, res) => {
  try {
    ...
    console.log(`✓ Age approved: ${rfid_uid} by ${req.user.email}`);
...
router.post('/add-credits', authenticateToken, async (req, res) => {
  try {
    ...
    console.log(`✓ Credits added: ${rfid_uid} +$${amount}`);
...
router.post('/redeem', authenticateToken, async (req, res) => {
  try {
    const { rfid_uid, amount = 7.00 } = req.body; // Default $7 per drink
    ...
    console.log(`✓ Drink served: ${rfid_uid} -$${amount}, balance: $${result.rows[0].credits}`);
